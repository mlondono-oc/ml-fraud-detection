"""
Preprocesamiento y features engineering para el dataset de
transacciones.

Consolida las transformaciones definidas en el análisis
exploratorio (notebooks/01_eda.ipynb)
"""
...
def preprocessing_pipeline(umbral_otros: int = UMBRAL_OTROS) -> Pipeline:
    ...
    categorical_transformer = Pipeline(
        steps=[
            (
                "sin_registro",
                SimpleImputer(strategy="constant", fill_value="sin_registro"),
            ),
            (
                "codigo",
                OrdinalEncoder(
